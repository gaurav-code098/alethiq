...
const AuthCard = ({ onClose }) => {
  ...
  return (
    <div className="w-full max-w-md p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl relative">
      ...
      </form>import React, { useState, useRef, useEffect, useMemo, createContext, useContext } from "react";
import { useStream } from "./hooks/useStream";
import { useTypewriter } from "./hooks/useTypewriter";
import ReactMarkdown from "react-markdown";
import { motion, AnimatePresence } from "framer-motion";
...
// --- 1. VISUAL: MQVERK BACKGROUND (Fixed) ---
const MqverkBackground = () => {
  return (
    <div className="fixed inset-0 w-full h-full -z-50 bg-[#050505]">
      {/* 1. Base Grid Layer - Slightly increased opacity for visibility */}
      <div 
        ...
        style={{
          ...
          // Subtle radial mask to fade edges (optional, removed for maximum visibility if needed)
      ...
      {/* 2. Noise Texture for "Film Grain" look */}
...
// --- 2. VISUAL: DECRYPT TEXT ANIMATION ---
const DecryptedText = ({ text, className }) => {
  ...
  useEffect(() => { animate(); return () => clearInterval(intervalRef.current); }, []);

  return (
    <span className={`${className} font-mono tracking-normal`} onMouseEnter={() => animate()}>
      {displayText}
    </span>
  );
};
...
const StockCard = ({ data }) => {
  ...
  return (
    <div className="my-6 p-5 bg-[#0e0e0e] rounded-xl border border-white/10 shadow-xl max-w-sm overflow-hidden relative group">
      ...
      <div className="grid grid-cols-2 gap-4 mt-4 pt-3 border-t border-white/5"><div><p className="text-[10px] text-zinc-600 uppercase">Mkt Cap</p><p className="text-xs text-zinc-300">{data.market_cap || data.marketCap || 'N/A'}</p></div><div className="text-right"><p className="text-[10px] text-zinc-600 uppercase">P/E Ratio</p><p className="text-xs text-zinc-300">{data.pe_ratio || data.peRatio || 'N/A'}</p></div></div>
    </div>
  );
};
  ...
  return (
    <div className="absolute top-6 right-6 z-50">
        <button onClick={() => setIsOpen(!isOpen)} className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center hover:bg-white/5 transition-all group shadow-2xl">
            <div className="w-8 h-8 rounded-full bg-teal-900/30 flex items-center justify-center border border-teal-500/20 text-teal-400 text-sm font-bold shadow-[0_0_15px_rgba(20,184,166,0.15)] group-hover:shadow-[0_0_20px_rgba(20,184,166,0.3)] transition-all">{displayName?.[0]?.toUpperCase() || <User size={14}/>}</div>
        </button>
        <AnimatePresence>
            {isOpen && (<motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="absolute top-full right-0 mt-3 w-56 bg-[#0a0a0a] border border-white/10 rounded-xl shadow-2xl overflow-hidden backdrop-blur-xl"><div className="px-5 py-4 border-b border-white/5"><p className="text-sm text-white font-medium">{displayName}</p><p className="text-xs text-zinc-500 mt-0.5 truncate">{user.email}</p></div><button onClick={logout} className="w-full flex items-center gap-3 px-5 py-3 text-xs text-red-400/80 hover:text-red-400 hover:bg-red-500/5 transition-colors"><LogOut size={14} /> Sign Out</button></motion.div>)}
        </AnimatePresence>
    </div>
  );
};
...
const AuthCard = ({ onClose }) => {
  ...
  return (
    <div className="w-full max-w-md p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl relative">
      <button onClick={onClose} className="absolute top-6 right-6 text-zinc-500 hover:text-white transition-colors"><X size={20} /></button>
      <div className="flex flex-col items-center gap-2 mb-8 text-center"><h1 className="text-2xl font-semibold text-white">{isSignUp ? "Create account" : "Welcome back"}</h1><p className="text-zinc-500 text-sm">{isSignUp ? "Join the Alethiq network" : "Sign in to your account"}</p></div>
...
const MarkdownComponents = { 
  h1: ({node, ...props}) => <h1 className="text-2xl md:text-3xl font-normal text-white mt-8 mb-4 tracking-tight font-mono break-words" {...props} />, 
  h2: ({node, ...props}) => <h2 className="text-lg md:text-xl font-normal text-zinc-200 mt-6 mb-3 tracking-wide flex items-center gap-2 font-mono break-words" {...props}><div className="w-1 h-5 bg-teal-500/50 rounded-full" />{props.children}</h2>, 
  p: ({node, children, ...props}) => <p className="text-zinc-300 leading-7 mb-4 text-[15px] md:text-[16px] font-normal tracking-wide whitespace-pre-wrap break-words" {...props}>{children}</p>, 
  ul: ({node, ...props}) => <ul className="space-y-2 mb-6 text-zinc-300 pl-2 break-words" {...props} />, 
  li: ({node, ...props}) => <li className="flex gap-3 items-start"><span className="mt-2 w-1.5 h-1.5 rounded-full bg-teal-500/50 flex-shrink-0" /><span className="flex-1 leading-7 break-words">{props.children}</span></li>, 
...
const AnswerSection = ({ data, isTyping, status }) => { 
    ...
    parts.forEach((part, index) => {
        if (index % 2 === 0) { if (part.trim()) mappedParts.push({ type: 'text', content: part }); } 
            ...
            try {
                ...
                if (jsonString.includes("noxious") || jsonString.includes("groq")) return;
                if (cardCount < 1) {
  ...
  return ( 
    <div className="relative group/answer w-full"> 
      ...
      <div className="prose prose-invert max-w-none prose-p:text-zinc-300 w-full break-words"> 
        {contentParts.map((part, idx) => ( 
            <React.Fragment key={idx}> 
                ...
                {part.type === 'stock-card' && <div className="my-6 animate-in fade-in slide-in-from-bottom-6 duration-700"><StockCard data={part.data} /></div>}
            </React.Fragment> 
        ))} 
      </div> 
...
const ContentBlock = ({ data, sources, images, isTyping, status, onRelatedClick }) => { 
  ...
  return ( 
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out mb-24 md:mb-32 w-full max-w-full"> 
      ...
      {!isTyping && relatedQuestions.length > 0 && ( <div className="mt-8 pt-8 border-t border-white/[0.05]"> <h3 className="text-xs font-mono uppercase tracking-[0.2em] text-zinc-600 mb-6 flex items-center gap-2"><Layers className="w-3 h-3" /> Explore Further</h3> <div className="flex flex-col gap-2"> {relatedQuestions.map((q, i) => ( <motion.button key={i} onClick={() => onRelatedClick(q)} whileHover={{ x: 4, backgroundColor: "rgba(255, 255, 255, 0.03)" }} className="group flex items-start justify-between w-full p-4 text-left rounded-xl border border-white/5 bg-white/[0.01] hover:border-white/10 transition-all"> <span className="text-zinc-400 group-hover:text-zinc-200 font-light text-sm tracking-wide break-words pr-4">{q}</span> <ArrowRight className="w-3 h-3 text-zinc-700 group-hover:text-teal-400 transition-colors opacity-0 group-hover:opacity-100 flex-shrink-0 mt-1" /> </motion.button> ))} </div> </div> )} 
    </div> 
  ); 
};

// 🟢 SEARCH FORM (Variable Height)
const SearchForm = ({ fixed = false, query, setQuery, handleSearch, isStreaming, stopStream, hasHistory, isSidebarOpen }) => {
    const textareaRef = useRef(null);
    const handleInput = (e) => { setQuery(e.target.value); if (textareaRef.current) { textareaRef.current.style.height = 'auto'; textareaRef.current.style.height = `${textareaRef.current.scrollHeight}px`; } };
    useEffect(() => { if (query === "" && textareaRef.current) textareaRef.current.style.height = 'auto'; }, [query]);
    const handleKeyDown = (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSearch(query); } };
    ...
    return (
        <div className={`w-full transition-all duration-300 ${fixed ? `fixed bottom-6 left-0 flex justify-center z-40 px-4 ${isSidebarOpen ? 'md:pl-[260px]' : 'md:pl-0'}` : ""}`}>
            <div className={`w-full max-w-3xl transition-all relative group`}>
                <form onSubmit={(e) => { e.preventDefault(); handleSearch(query); }} className={`relative flex items-end rounded-3xl transition-all duration-300 ${fixed ? "bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 p-2 shadow-2xl" : "bg-white/[0.03] border border-white/5 hover:border-white/10 hover:bg-white/[0.05] px-5 py-3 shadow-[0_0_40px_-10px_rgba(0,0,0,0.5)]"}`}>
                    <div className={`flex items-end w-full ${fixed ? "pl-3" : ""}`}>
                        {!fixed && <Search className="w-4 h-4 text-zinc-500 mr-4 opacity-50 mb-3" />}
                        <textarea ref={textareaRef} rows={1} className={`w-full bg-transparent text-zinc-200 outline-none placeholder:text-zinc-600/70 tracking-wide font-mono resize-none overflow-hidden py-2 ${fixed ? "text-base max-h-[150px]" : "text-sm md:text-base max-h-[200px]"}`} placeholder={fixed ? "Ask a follow-up..." : "What do you want to discover?"} value={query} onChange={handleInput} onKeyDown={handleKeyDown} autoFocus={!hasHistory} />
...
function App() {
  ...
  useEffect(() => { const fetchSuggestions = async () => { try { const res = await fetch(`${SUGGESTIONS_URL}/get-suggestions`); if (res.ok) { setSuggestions(await res.json()); } else throw new Error(); } catch (e) { const fallback = ["James Webb Telescope", "Generative UI Design", "Terraforming Mars", "6G Networks"].sort(() => 0.5 - Math.random()); setSuggestions(fallback.slice(0, 4)); } finally { setLoadingSuggestions(false); } }; fetchSuggestions(); }, []); 
  ...
  const handleSearch = (searchQuery) => { if (!searchQuery?.trim()) return; const currentHistory = [...chatHistory]; setChatHistory(prev => [...prev, { type: 'user', content: searchQuery }]); setQuery(""); setLastQuery(searchQuery); setAutoScroll(true); streamData(searchQuery, "fast", currentHistory); };
  ...
  return (
    // ⚠️ FIXED: Removed bg-[#050505] so the grid is visible!
    <div className="min-h-screen w-full text-zinc-100 flex overflow-hidden relative antialiased selection:bg-teal-500/20 selection:text-teal-200 font-sans">
      <MqverkBackground />
      ...
      <motion.div animate={{ width: isSidebarOpen ? 260 : 0, opacity: isSidebarOpen ? 1 : 0 }} className={`fixed md:relative top-0 left-0 h-screen bg-[#050505] border-r border-white/5 z-50 flex flex-col overflow-hidden`}>
           <div className="w-[260px] flex flex-col h-full p-4"><div className="flex justify-between items-center mb-10 px-2 mt-2"><div className="font-display text-lg tracking-tight flex items-center gap-2 text-zinc-100"><div className="w-2 h-2 rounded-full bg-teal-500 shadow-[0_0_10px_rgba(20,184,166,0.5)]" />Alethiq</div><button onClick={() => setIsSidebarOpen(false)} className="text-zinc-600 hover:text-white transition-colors"><PanelLeftClose size={16} /></button></div><button onClick={handleNewChat} className="flex items-center gap-3 w-full bg-white/5 hover:bg-white/10 text-white rounded-xl px-4 py-3 transition-all border border-white/5 group mb-8"><Plus size={16} className="text-zinc-400 group-hover:text-white" /><span className="text-sm font-medium tracking-wide">New Thread</span></button><div className="flex-1 overflow-y-auto px-2 space-y-1">{user ? (<div className="flex items-center justify-between px-2 mb-3"><span className="text-[10px] font-mono text-zinc-600 uppercase tracking-[0.2em]">History</span><button onClick={fetchHistory} className="text-zinc-600 hover:text-white transition-colors"><RefreshCw size={10} /></button></div>) : <button onClick={() => setShowAuthModal(true)} className="flex items-center gap-3 w-full bg-teal-900/10 hover:bg-teal-900/20 text-teal-400 rounded-xl px-4 py-3 border border-teal-500/10 transition-all text-sm font-medium"><LogIn size={16} /> <span className="opacity-80">Sign In / Sign Up</span></button>}{user && threads.length > 0 ? threads.map((thread) => { let cleanTitle = thread.title; try { const parsed = JSON.parse(thread.title); if (parsed.query) cleanTitle = parsed.query; } catch (e) { } return (<button key={thread.id} onClick={() => handleLoadThread(thread)} className="w-full text-left p-3 rounded-lg hover:bg-white/5 text-zinc-500 hover:text-zinc-200 transition-colors text-xs truncate flex items-center gap-3 group"><MessageSquare size={14} className="text-zinc-700 group-hover:text-zinc-400 transition-colors" /><span className="truncate">{cleanTitle || "Untitled Chat"}</span></button>); }) : user && <div className="flex flex-col items-center justify-center pt-24 opacity-30"><History className="w-6 h-6 text-zinc-500 mb-3" /><p className="text-zinc-500 text-[10px] text-center font-mono">NO HISTORY</p></div>}</div><div className="mt-4 pt-4 border-t border-white/5 text-[10px] text-zinc-700 text-center font-mono tracking-[0.2em] opacity-50">v2.0</div></div>
      </motion.div>
      <div ref={mainScrollRef} onScroll={handleScroll} className="flex-1 flex flex-col h-screen relative w-full overflow-y-auto scroll-smooth bg-transparent overflow-x-hidden">
           <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-30 pointer-events-none"><div className="pointer-events-auto">{!isSidebarOpen && <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-transparent text-zinc-600 hover:text-white transition-colors"><PanelLeftOpen size={20} /></button>}</div><div className="pointer-events-auto">{!user && !isSidebarOpen && <button onClick={() => setShowAuthModal(true)} className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-xs text-zinc-300 hover:text-white hover:bg-white/10 transition-all font-mono tracking-wide"><LogIn size={12} /> ACCESS</button>}{user && <HeaderProfile />}</div></div>
           {!hasHistory && (<div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.2, ease: "easeOut" }} className="w-full max-w-3xl flex flex-col items-center text-center z-10"><div className="mb-12 flex flex-col items-center"><motion.h1 initial={{ letterSpacing: "0.1em", opacity: 0, y: 20 }} animate={{ letterSpacing: "0.2em", opacity: 1, y: 0 }} transition={{ duration: 1, ease: "easeOut" }} className="text-6xl md:text-8xl font-light tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-b from-white via-zinc-400 to-zinc-800 font-mono mb-6 select-none"><DecryptedText text="ALETHIQ" className="font-mono" /></motion.h1><motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 1 }} className="text-[10px] md:text-xs text-zinc-600 font-mono tracking-[0.4em] uppercase">Intelligence Search Engine</motion.p></div><motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3, duration: 0.6 }} className="w-full mb-12"><SearchForm fixed={false} query={query} setQuery={setQuery} handleSearch={handleSearch} isStreaming={isStreaming} stopStream={stopStream} hasHistory={hasHistory} isSidebarOpen={isSidebarOpen} /></motion.div><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 1 }} className="flex flex-wrap justify-center gap-3">{loadingSuggestions ? [...Array(4)].map((_, i) => <div key={i} className="h-6 w-24 bg-white/5 animate-pulse rounded-full" />) : suggestions.map((s, i) => (<button key={i} onClick={() => handleSearch(s)} className="text-[10px] text-zinc-500 hover:text-zinc-300 border border-white/5 hover:border-white/20 px-3 py-1.5 rounded-full transition-all uppercase tracking-wider font-mono hover:bg-white/[0.02]">{s}</button>))}</motion.div></motion.div></div>)}
           {hasHistory && (<div className="w-full max-w-3xl mx-auto px-4 md:px-6 pt-32 pb-48"><div className="space-y-20">{chatHistory.map((msg, idx) => (<div key={idx} className="group">{msg.type === 'user' ? (<h2 className="text-2xl md:text-4xl font-light text-white/90 tracking-tight mb-8 leading-snug font-mono break-words">{msg.content}</h2>) : (<ContentBlock data={msg.content} sources={msg.sources} images={msg.images} isTyping={false} onRelatedClick={handleSearch} />)}</div>))}{isStreaming && <ContentBlock data={data} sources={sources} images={images} isTyping={true} status={status} onRelatedClick={handleSearch} />}<div ref={messagesEndRef} className="h-4" /></div><SearchForm fixed={true} query={query} setQuery={setQuery} handleSearch={handleSearch} isStreaming={isStreaming} stopStream={stopStream} hasHistory={hasHistory} isSidebarOpen={isSidebarOpen} /></div>)}
        </div>
      </div>
  );
}
