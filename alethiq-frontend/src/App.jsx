...
import { 
  ...
  Paperclip, Image as ImageIcon // 🟢 Added Icons
} from "lucide-react";
...
// 🟢 FIX 2: SEARCH FORM (Variable Height Textarea + Image Upload)
const SearchForm = ({ fixed = false, query, setQuery, handleSearch, isStreaming, stopStream, hasHistory, isSidebarOpen }) => {
    ...
    const onSubmit = () => {
        ...
        handleSearch(query, selectedImage); 
        setQuery("");
    ...
    return (
        <div className={`w-full transition-all duration-300 ${fixed ? `fixed bottom-6 left-0 flex justify-center z-40 px-4 ${isSidebarOpen ? 'md:pl-[260px]' : 'md:pl-0'}` : ""}`}>
            <div className={`w-full max-w-3xl transition-all relative group flex flex-col items-center`}>
                
                {/* 🟢 IMAGE PREVIEW (If selected) */}
                {selectedImage && (
                    <div className="relative mb-2 self-start bg-[#1a1a1a] p-2 rounded-xl border border-white/10 animate-in fade-in slide-in-from-bottom-2">
                        <img src={selectedImage} alt="Preview" className="h-16 w-16 object-cover rounded-lg" />
                        <button onClick={() => setSelectedImage(null)} className="absolute -top-2 -right-2 bg-zinc-800 text-zinc-400 hover:text-white rounded-full p-1 border border-white/10"><X size={12} /></button>
                    </div>
                )}

                <div className={`relative flex items-end w-full rounded-3xl transition-all duration-300 ${fixed ? "bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/10 p-2 shadow-2xl" : "bg-white/[0.03] border border-white/5 hover:border-white/10 hover:bg-white/[0.05] px-5 py-3 shadow-[0_0_40px_-10px_rgba(0,0,0,0.5)]"}`}>
                    
                    {/* 🟢 PAPERCLIP ICON */}
                    <div className="flex items-center pb-2 pl-1 pr-2 mb-1">
                        <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*" />
                        <button type="button" onClick={() => fileInputRef.current.click()} className="text-zinc-500 hover:text-teal-400 transition-colors"><Paperclip size={18} /></button>
                    </div>
                    ...
                    <div className="flex items-center justify-center pb-2 pl-2 mb-1">
                        {isStreaming ? ( 
                            <button type="button" onClick={stopStream} className="w-8 h-8 flex items-center justify-center rounded-full text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-all duration-300"><Square size={12} fill="currentColor" /></button> 
                        ) : ( 
                            <button type="button" onClick={onSubmit} disabled={!query.trim() && !selectedImage} className="w-8 h-8 flex items-center justify-center rounded-full text-zinc-600 hover:text-zinc-200 hover:bg-white/10 transition-all duration-300 disabled:opacity-0 disabled:hover:bg-transparent"><ArrowRight size={16} /></button> 
                        )}
                    </div>
                </div>
            </div>
...
function App() {
  ...
  // 🟢 UPDATED: Handle Search now accepts Image
  const handleSearch = (searchQuery, image = null) => { 
    if (!searchQuery?.trim() && !image) return; 
    const currentHistory = [...chatHistory]; 
    setChatHistory(prev => [...prev, { type: 'user', content: searchQuery, image: image }]); // Store image in history if needed
    setQuery(""); 
    setLastQuery(searchQuery); 
    setAutoScroll(true); 
    // 🟢 PASS IMAGE TO STREAM
